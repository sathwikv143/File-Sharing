...
session_start();
$user = "/PHPCollege/SampleProject/ChatApp/UserFolders/".$_SESSION['LoggedUser'];

// Root path for file manager
$root_path = $_SERVER['DOCUMENT_ROOT'].$user;
...
// link to parent folder
        ...
        foreach ($folders as $f) {
            ...
            foreach ($files as $f) {
                if($f!="index.php" && $f!="NewFileManager.php")
                {
                    $is_link = is_link($path . '/' . $f);
...
function fm_show_nav_path($path)
{
    ...
    <div class="path">
        ...
        <?php
        $path = fm_clean_path($path);
        $root_url = "<a href='?p='><i class='icon-home'></i></a>";
